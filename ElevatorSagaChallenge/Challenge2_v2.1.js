...
{ 
    ...
    init: function(elevators, floors) 
    {
        ...
        elevators.forEach(function(elevator, ind)
        {
            ...
            {
                //window.alert("idle")
                if ((masterUpBuffer.length != 0 || masterDownBuffer.length != 0) && elevator.loadFactor() < (1/3))          {
            ...
            elevator.on("passing_floor", function(floorNum, direction)
            {
                ...
                else if(direction == "up" && elevator.loadFactor() < (1/3) && masterUpBuffer.find(function(floor)
                {
                    return floorNum == floor;
                }))
                    elevator.goToFloor(floorNum, true);
                else if(direction == "down" && elevator.loadFactor() < (1/3) && masterDownBuffer.find(function(floor)
                {
                    return floorNum == floor;
                }))
                    elevator.goToFloor(floorNum, true);
